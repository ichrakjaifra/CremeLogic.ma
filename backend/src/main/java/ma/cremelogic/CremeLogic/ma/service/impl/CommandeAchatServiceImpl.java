...
import ma.cremelogic.CremeLogic.ma.dto.request.LigneReceptionRequest;
...
public class CommandeAchatServiceImpl implements CommandeAchatService {
        ...
        public CommandeAchatResponse recevoirCommande(Long id, ReceptionCommandeRequest request) {
                ...
                for (LigneReceptionRequest reception : request.getLignesRecues()) {
                        LigneCommande ligne = commande.getLignesCommande().stream()
                                        .filter(l -> l.getId().equals(reception.getLigneCommandeId()))
                                        .findFirst()
                                        .orElseThrow(() -> new ResourceNotFoundException("Ligne de commande", "id",
                                                        reception.getLigneCommandeId()));

                        BigDecimal quantiteRecue = reception.getQuantiteRecue();

                        if (quantiteRecue.compareTo(
                                        ligne.getQuantiteCommandee().subtract(ligne.getQuantiteRecue())) > 0) {
                                throw new ValidationException(
                                                "Quantité reçue supérieure à la quantité restante pour l'ingrédient: " +
        ...
        private CommandeAchatResponse mapToResponse(CommandeAchat commande) {
                ...
                return CommandeAchatResponse.builder()
                                ...
                                .createurNom(commande.getCreateur() != null ? commande.getCreateur().getNom() + " "
                                                + commande.getCreateur().getPrenom() : null)
